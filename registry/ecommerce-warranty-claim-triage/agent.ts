...
/** Warranty Claim Triage — v1 validated. Pain: Warranty intake */
...
const skill = {
  ...
  systemPrompt: `You are Warranty Claim Triage. Warranty intake. Output: Triage typed.
...
export function createEcommerceWarrantyClaimTriageAgent(config: EcommerceWarrantyClaimTriageConfig) {
  ...
  return {
    name: 'ecommerce-warranty-claim-triage',
    run,
    asHandle() { return { name: 'ecommerce-warranty-claim-triage', run: (t: string) => run(t).then((r) => JSON.stringify(r)) } },
  }
}
