...
/**
 * Warranty Claim Triage — Triage typed
 * Pain: Warranty intake
...
export interface EcommerceWarrantyClaimTriageConfig {
...
const skill = {
  ...
  description: "Warranty Claim Triage — typed output agent (draft spec).",
  systemPrompt: `You are Warranty Claim Triage. Warranty intake. Expected output: Triage typed.
...
Call submit_claim_triage exactly once with the structured result. Stop.`,
  tools: ['submit_claim_triage'],
}

export function createEcommerceWarrantyClaimTriageAgent(config: EcommerceWarrantyClaimTriageConfig) {
  const emit = (label: string, status: 'start' | 'ok' | 'skip' | 'error', detail?: string) => {
    for (const o of config.observers ?? []) void o.on({ type: 'progress', label, status, detail })
  }
  const submit = (): ToolDefinition =>
    defineZodTool({
      name: 'submit_claim_triage',
  ...
  async function run(input: string): Promise<AgentResult> {
    if (!input?.trim()) throw new Error('ecommerce-warranty-claim-triage requires non-empty input')
    emit('run', 'start')
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'ecommerce-warranty-claim-triage', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
