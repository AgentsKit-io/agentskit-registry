...
/**
 * Pricing Calculator Memo — Memo typed
 * Pain: Pricing opaque
...
const skill = {
  ...
  systemPrompt: `You are Pricing Calculator Memo. Pricing opaque. Expected output: Memo typed.
...
export function createSalesPricingCalculatorMemoAgent(config: SalesPricingCalculatorMemoConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'sales-pricing-calculator-memo', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
