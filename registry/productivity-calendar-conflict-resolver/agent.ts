...
/** Calendar Conflict Resolver — v1 validated. Pain: Scheduling conflicts */
...
const skill = {
  ...
  systemPrompt: `You are Calendar Conflict Resolver. Scheduling conflicts. Output: Options typed.
Draft sections with citations from input. Gaps for missing facts.
NEVER invent facts — gaps and openQuestions for missing input. Always draft for human review.
${UNTRUSTED_CONTENT_DIRECTIVE}
Call submit_conflict_resolver exactly once. Stop.`,
...
export function createProductivityCalendarConflictResolverAgent(config: ProductivityCalendarConflictResolverConfig) {
  const submit = (): ToolDefinition =>
    defineZodTool({ name: 'submit_conflict_resolver', description: 'Submit result. Once.', schema: Output, toJsonSchema: toJson, async execute() { return 'recorded' } }) as ToolDefinition
  ...
  return {
    name: 'productivity-calendar-conflict-resolver',
    run,
    asHandle() { return { name: 'productivity-calendar-conflict-resolver', run: (t: string) => run(t).then((r) => JSON.stringify(r)) } },
  }
}
