...
/**
 * Database Query Reviewer — SQL findings typed
 * Pain: N+1 and missing indexes
...
const skill = {
  ...
  systemPrompt: `You are Database Query Reviewer. N+1 and missing indexes. Expected output: SQL findings typed.
...
Call submit_query_reviewer exactly once with the structured result. Stop.`,
...
export function createCodingDatabaseQueryReviewerAgent(config: CodingDatabaseQueryReviewerConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'coding-database-query-reviewer', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
