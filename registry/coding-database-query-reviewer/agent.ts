...
/** Database Query Reviewer — v1 validated. Pain: N+1 and missing indexes */
...
const skill = {
  ...
  description: "Database Query Reviewer — typed output agent (draft spec).",
  systemPrompt: `You are Database Query Reviewer. N+1 and missing indexes. Output: SQL findings typed.
...
export function createCodingDatabaseQueryReviewerAgent(config: CodingDatabaseQueryReviewerConfig) {
  const submit = (): ToolDefinition =>
    defineZodTool({ name: 'submit_query_reviewer', description: 'Submit result. Once.', schema: Output, toJsonSchema: toJson, async execute() { return 'recorded' } }) as ToolDefinition
  ...
  return {
    name: 'coding-database-query-reviewer',
    run,
    asHandle() { return { name: 'coding-database-query-reviewer', run: (t: string) => run(t).then((r) => JSON.stringify(r)) } },
  }
}
