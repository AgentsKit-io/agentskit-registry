...
/** Bundle Suggester — v1 validated. Pain: Bundles manual */
...
const skill = {
  ...
  systemPrompt: `You are Bundle Suggester. Bundles manual. Output: Bundles typed.
Draft sections with citations from input. Gaps for missing facts.
NEVER invent facts — gaps and openQuestions for missing input. Always draft for human review.
${UNTRUSTED_CONTENT_DIRECTIVE}
Call submit_bundle_suggester exactly once. Stop.`,
...
export function createEcommerceBundleSuggesterAgent(config: EcommerceBundleSuggesterConfig) {
  const submit = (): ToolDefinition =>
    defineZodTool({ name: 'submit_bundle_suggester', description: 'Submit result. Once.', schema: Output, toJsonSchema: toJson, async execute() { return 'recorded' } }) as ToolDefinition
  ...
  return {
    name: 'ecommerce-bundle-suggester',
    run,
    asHandle() { return { name: 'ecommerce-bundle-suggester', run: (t: string) => run(t).then((r) => JSON.stringify(r)) } },
  }
}
