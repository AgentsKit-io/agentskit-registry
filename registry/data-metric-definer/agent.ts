...
/** Metric Definer — v1 validated. Pain: Metric definitions vague */
...
const skill = {
  ...
  systemPrompt: `You are Metric Definer. Metric definitions vague. Output: Definition typed.
Draft sections with citations from input. Gaps for missing facts.
NEVER invent facts — gaps and openQuestions for missing input. Always draft for human review.
${UNTRUSTED_CONTENT_DIRECTIVE}
Call submit_metric_definer exactly once. Stop.`,
  tools: ['submit_metric_definer'],
}

export function createDataMetricDefinerAgent(config: DataMetricDefinerConfig) {
  const submit = (): ToolDefinition =>
    defineZodTool({ name: 'submit_metric_definer', description: 'Submit result. Once.', schema: Output, toJsonSchema: toJson, async execute() { return 'recorded' } }) as ToolDefinition
  ...
  return {
    name: 'data-metric-definer',
    run,
    asHandle() { return { name: 'data-metric-definer', run: (t: string) => run(t).then((r) => JSON.stringify(r)) } },
  }
}
