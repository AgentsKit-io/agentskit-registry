...
/**
 * Audit Evidence Collector — Evidence map typed
 * Pain: Audit prep chaotic
...
const skill = {
  ...
  systemPrompt: `You are Audit Evidence Collector. Audit prep chaotic. Expected output: Evidence map typed.
...
Call submit_evidence_collector exactly once with the structured result. Stop.`,
...
export function createOpsAuditEvidenceCollectorAgent(config: OpsAuditEvidenceCollectorConfig) {
  ...
  const submit = (): ToolDefinition =>
    defineZodTool({
      name: 'submit_evidence_collector',
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'ops-audit-evidence-collector', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
