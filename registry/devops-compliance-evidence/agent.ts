...
/**
 * Compliance Evidence — Evidence typed
 * Pain: SOC2 evidence
...
const skill = {
  ...
  systemPrompt: `You are Compliance Evidence. SOC2 evidence. Expected output: Evidence typed.
...
Call submit_compliance_evidence exactly once with the structured result. Stop.`,
...
export function createDevopsComplianceEvidenceAgent(config: DevopsComplianceEvidenceConfig) {
  ...
  const submit = (): ToolDefinition =>
    defineZodTool({
      name: 'submit_compliance_evidence',
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'devops-compliance-evidence', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
