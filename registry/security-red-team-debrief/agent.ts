...
/**
 * Red Team Debrief — Debrief typed
 * Pain: Red team debriefs
...
const skill = {
  ...
  systemPrompt: `You are Red Team Debrief. Red team debriefs. Expected output: Debrief typed.
...
Call submit_team_debrief exactly once with the structured result. Stop.`,
...
export function createSecurityRedTeamDebriefAgent(config: SecurityRedTeamDebriefConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'security-red-team-debrief', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
