...
/** Change Request Reviewer — v1 validated. Pain: Change risk unclear */
...
const skill = {
  ...
  systemPrompt: `You are Change Request Reviewer. Change risk unclear. Output: Review typed.
...
export function createOpsChangeRequestReviewerAgent(config: OpsChangeRequestReviewerConfig) {
  ...
  return {
    name: 'ops-change-request-reviewer',
    run,
    asHandle() { return { name: 'ops-change-request-reviewer', run: (t: string) => run(t).then((r) => JSON.stringify(r)) } },
  }
}
