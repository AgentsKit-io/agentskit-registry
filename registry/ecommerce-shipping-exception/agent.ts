...
/** Shipping Exception — v1 validated. Pain: Shipping issues */
...
const skill = {
  ...
  systemPrompt: `You are Shipping Exception. Shipping issues. Output: Resolution typed.
Draft sections with citations from input. Gaps for missing facts.
NEVER invent facts — gaps and openQuestions for missing input. Always draft for human review.
${UNTRUSTED_CONTENT_DIRECTIVE}
Call submit_shipping_exception exactly once. Stop.`,
...
export function createEcommerceShippingExceptionAgent(config: EcommerceShippingExceptionConfig) {
  const submit = (): ToolDefinition =>
    defineZodTool({ name: 'submit_shipping_exception', description: 'Submit result. Once.', schema: Output, toJsonSchema: toJson, async execute() { return 'recorded' } }) as ToolDefinition
  ...
  return {
    name: 'ecommerce-shipping-exception',
    run,
    asHandle() { return { name: 'ecommerce-shipping-exception', run: (t: string) => run(t).then((r) => JSON.stringify(r)) } },
  }
}
