...
/**
 * Meta Ad Copy — 3 variants typed + policy flags
 * Pain: Meta ads off-policy
...
const skill = {
  ...
  systemPrompt: `You are Meta Ad Copy. Meta ads off-policy. Expected output: 3 variants typed + policy flags.
...
export function createMarketingMetaAdCopyAgent(config: MarketingMetaAdCopyConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'marketing-meta-ad-copy', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
