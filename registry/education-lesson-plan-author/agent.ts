...
/**
 * Lesson Plan Author — Plan typed
 * Pain: Lesson planning slow
...
const skill = {
  ...
  systemPrompt: `You are Lesson Plan Author. Lesson planning slow. Expected output: Plan typed.
...
Call submit_plan_author exactly once with the structured result. Stop.`,
...
export function createEducationLessonPlanAuthorAgent(config: EducationLessonPlanAuthorConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'education-lesson-plan-author', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
