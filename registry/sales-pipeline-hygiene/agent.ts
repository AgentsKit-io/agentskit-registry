...
const skill = {
  ...
  systemPrompt: `You are Pipeline Hygiene. Dirty CRM. Expected output: Issues typed.
...
Call submit_pipeline_hygiene exactly once with the structured result. Stop.`,
...
export function createSalesPipelineHygieneAgent(config: SalesPipelineHygieneConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'sales-pipeline-hygiene', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
