...
/**
 * Competitive Feature Gap — Gap analysis typed
 * Pain: Feature gaps unclear
...
const skill = {
  ...
  systemPrompt: `You are Competitive Feature Gap. Feature gaps unclear. Expected output: Gap analysis typed.
...
Call submit_feature_gap exactly once with the structured result. Stop.`,
...
export function createProductCompetitiveFeatureGapAgent(config: ProductCompetitiveFeatureGapConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'product-competitive-feature-gap', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
