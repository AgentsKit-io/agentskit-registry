...
const model = (payload: Record<string, unknown>) =>
  mockAdapter({ response: () => [{ type: 'tool_call', toolCall: { id: '1', name: 'submit_corpus_scanner', args: JSON.stringify(payload) } }, { type: 'done' }] })
...
describe('ecosystem-doc-bridge-corpus-scanner', () => {
  ...
  it('escalates critical red flags', async () => {
    const r = await createEcosystemDocBridgeCorpusScannerAgent({ adapter: model({ category: 'x', severity: 'low', queue: 'q', rationale: 'm', gaps: [], openQuestions: [] }) }).run('full outage for all users')
