...
/** Doc-bridge Handoff Author — v1 validated. Pain: Agent handoffs between doc-bridge index and human adapters */
...
const skill = {
  ...
  systemPrompt: `You are Doc-bridge Handoff Author. Agent handoffs between doc-bridge index and human adapters. Output: Handoff doc typed per agent-handoff-v1 schema.
...
export function createEcosystemDocBridgeHandoffAuthorAgent(config: EcosystemDocBridgeHandoffAuthorConfig) {
  const submit = (): ToolDefinition =>
    defineZodTool({ name: 'submit_handoff_author', description: 'Submit result. Once.', schema: Output, toJsonSchema: toJson, async execute() { return 'recorded' } }) as ToolDefinition
  ...
  return {
    name: 'ecosystem-doc-bridge-handoff-author',
    run,
    asHandle() { return { name: 'ecosystem-doc-bridge-handoff-author', run: (t: string) => run(t).then((r) => JSON.stringify(r)) } },
  }
}
