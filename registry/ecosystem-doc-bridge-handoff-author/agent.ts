...
/**
 * Doc-bridge Handoff Author — Handoff doc typed per agent-handoff-v1 schema
...
const skill = {
  ...
  systemPrompt: `You are Doc-bridge Handoff Author. Agent handoffs between doc-bridge index and human adapters. Expected output: Handoff doc typed per agent-handoff-v1 schema.
...
Call submit_handoff_author exactly once with the structured result. Stop.`,
...
export function createEcosystemDocBridgeHandoffAuthorAgent(config: EcosystemDocBridgeHandoffAuthorConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'ecosystem-doc-bridge-handoff-author', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
