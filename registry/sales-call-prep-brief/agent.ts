...
/** Call Prep Brief — v1 validated. Pain: Unprepared calls */
...
const skill = {
  ...
  systemPrompt: `You are Call Prep Brief. Unprepared calls. Output: Brief typed.
Draft sections with citations from input. Gaps for missing facts.
NEVER invent facts — gaps and openQuestions for missing input. Always draft for human review.
${UNTRUSTED_CONTENT_DIRECTIVE}
Call submit_prep_brief exactly once. Stop.`,
  tools: ['submit_prep_brief'],
}

export function createSalesCallPrepBriefAgent(config: SalesCallPrepBriefConfig) {
  const submit = (): ToolDefinition =>
    defineZodTool({ name: 'submit_prep_brief', description: 'Submit result. Once.', schema: Output, toJsonSchema: toJson, async execute() { return 'recorded' } }) as ToolDefinition
  ...
  return {
    name: 'sales-call-prep-brief',
    run,
    asHandle() { return { name: 'sales-call-prep-brief', run: (t: string) => run(t).then((r) => JSON.stringify(r)) } },
  }
}
