...
/**
 * Adverse Event Reporter — Report draft typed
 * Pain: AE reporting slow
...
const skill = {
  ...
  systemPrompt: `You are Adverse Event Reporter. AE reporting slow. Expected output: Report draft typed.
...
Call submit_event_reporter exactly once with the structured result. Stop.`,
...
export function createClinicalAdverseEventReporterAgent(config: ClinicalAdverseEventReporterConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'clinical-adverse-event-reporter', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
