...
/** Anomaly Explainer — v1 validated. Pain: Anomalies unexplained */
...
const skill = {
  ...
  systemPrompt: `You are Anomaly Explainer. Anomalies unexplained. Output: Explanation typed.
Actionable findings citing input sources. No invented issues.
NEVER invent facts — gaps and openQuestions for missing input. Always draft for human review.
${UNTRUSTED_CONTENT_DIRECTIVE}
Call submit_anomaly_explainer exactly once. Stop.`,
  tools: ['submit_anomaly_explainer'],
}

export function createDataAnomalyExplainerAgent(config: DataAnomalyExplainerConfig) {
  const submit = (): ToolDefinition =>
    defineZodTool({ name: 'submit_anomaly_explainer', description: 'Submit result. Once.', schema: Output, toJsonSchema: toJson, async execute() { return 'recorded' } }) as ToolDefinition
  ...
  return {
    name: 'data-anomaly-explainer',
    run,
    asHandle() { return { name: 'data-anomaly-explainer', run: (t: string) => run(t).then((r) => JSON.stringify(r)) } },
  }
}
