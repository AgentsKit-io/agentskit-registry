...
/**
 * LGPD Assessor — Assessment typed
 * Pain: LGPD gap analysis
...
const skill = {
  ...
  systemPrompt: `You are LGPD Assessor. LGPD gap analysis. Expected output: Assessment typed.
...
Call submit_lgpd_assessor exactly once with the structured result. Stop.`,
...
export function createComplianceLgpdAssessorAgent(config: ComplianceLgpdAssessorConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'compliance-lgpd-assessor', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
