...
/**
 * Deploy Risk Reviewer — Risk typed
 * Pain: Risky deploys
...
const skill = {
  ...
  systemPrompt: `You are Deploy Risk Reviewer. Risky deploys. Expected output: Risk typed.
...
Call submit_risk_reviewer exactly once with the structured result. Stop.`,
...
export function createDevopsDeployRiskReviewerAgent(config: DevopsDeployRiskReviewerConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'devops-deploy-risk-reviewer', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
