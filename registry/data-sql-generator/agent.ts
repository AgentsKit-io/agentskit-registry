...
/**
 * SQL Generator — SQL typed
 * Pain: SQL from questions
...
const skill = {
  ...
  systemPrompt: `You are SQL Generator. SQL from questions. Expected output: SQL typed.
...
Call submit_sql_generator exactly once with the structured result. Stop.`,
...
export function createDataSqlGeneratorAgent(config: DataSqlGeneratorConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'data-sql-generator', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
