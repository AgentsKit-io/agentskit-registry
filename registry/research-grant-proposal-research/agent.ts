...
/**
 * Grant Proposal Research — Literature typed
 * Pain: Grant background slow
...
const skill = {
  ...
  systemPrompt: `You are Grant Proposal Research. Grant background slow. Expected output: Literature typed.
...
Call submit_proposal_research exactly once with the structured result. Stop.`,
...
export function createResearchGrantProposalResearchAgent(config: ResearchGrantProposalResearchConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'research-grant-proposal-research', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
