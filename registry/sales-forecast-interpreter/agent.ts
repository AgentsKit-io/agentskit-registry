...
/** Forecast Interpreter — v1 validated. Pain: Forecast opaque */
...
const skill = {
  ...
  systemPrompt: `You are Forecast Interpreter. Forecast opaque. Output: Insights typed.
Actionable findings citing input sources. No invented issues.
NEVER invent facts — gaps and openQuestions for missing input. Always draft for human review.
${UNTRUSTED_CONTENT_DIRECTIVE}
Call submit_forecast_interpreter exactly once. Stop.`,
...
export function createSalesForecastInterpreterAgent(config: SalesForecastInterpreterConfig) {
  const submit = (): ToolDefinition =>
    defineZodTool({ name: 'submit_forecast_interpreter', description: 'Submit result. Once.', schema: Output, toJsonSchema: toJson, async execute() { return 'recorded' } }) as ToolDefinition
  ...
  return {
    name: 'sales-forecast-interpreter',
    run,
    asHandle() { return { name: 'sales-forecast-interpreter', run: (t: string) => run(t).then((r) => JSON.stringify(r)) } },
  }
}
