...
/**
 * Lab Interpreter — Interpretation typed
 * Pain: Lab results hard to scan
...
const skill = {
  ...
  systemPrompt: `You are Lab Interpreter. Lab results hard to scan. Expected output: Interpretation typed.
...
Call submit_lab_interpreter exactly once with the structured result. Stop.`,
...
export function createClinicalLabInterpreterAgent(config: ClinicalLabInterpreterConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'clinical-lab-interpreter', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
