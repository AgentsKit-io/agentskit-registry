...
/** Renewal Risk Scorer — v1 validated. Pain: Renewal risk hidden */
...
const skill = {
  ...
  systemPrompt: `You are Renewal Risk Scorer. Renewal risk hidden. Output: Score typed.
...
export function createSalesRenewalRiskScorerAgent(config: SalesRenewalRiskScorerConfig) {
  ...
  return {
    name: 'sales-renewal-risk-scorer',
    run,
    asHandle() { return { name: 'sales-renewal-risk-scorer', run: (t: string) => run(t).then((r) => JSON.stringify(r)) } },
  }
}
