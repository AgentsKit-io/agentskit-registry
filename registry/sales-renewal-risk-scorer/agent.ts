...
/**
 * Renewal Risk Scorer — Score typed
 * Pain: Renewal risk hidden
...
export interface SalesRenewalRiskScorerConfig {
...
const skill = {
  ...
  description: "Renewal Risk Scorer — typed output agent (draft spec).",
  systemPrompt: `You are Renewal Risk Scorer. Renewal risk hidden. Expected output: Score typed.
...
Call submit_risk_scorer exactly once with the structured result. Stop.`,
  tools: ['submit_risk_scorer'],
}

export function createSalesRenewalRiskScorerAgent(config: SalesRenewalRiskScorerConfig) {
  const emit = (label: string, status: 'start' | 'ok' | 'skip' | 'error', detail?: string) => {
    for (const o of config.observers ?? []) void o.on({ type: 'progress', label, status, detail })
  }
  const submit = (): ToolDefinition =>
    defineZodTool({
      name: 'submit_risk_scorer',
  ...
  async function run(input: string): Promise<AgentResult> {
    if (!input?.trim()) throw new Error('sales-renewal-risk-scorer requires non-empty input')
    emit('run', 'start')
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'sales-renewal-risk-scorer', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
