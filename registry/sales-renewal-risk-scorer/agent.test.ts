...
describe('sales-renewal-risk-scorer', () => {
  ...
  it('escalates critical red flags', async () => {
    const r = await createSalesRenewalRiskScorerAgent({ adapter: model({ category: 'x', severity: 'low', queue: 'q', rationale: 'm', gaps: [], openQuestions: [] }) }).run('full outage for all users')
