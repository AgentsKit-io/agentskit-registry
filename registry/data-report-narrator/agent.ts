...
/**
 * Report Narrator — Narrative typed
 * Pain: Reports need narrative
...
const skill = {
  ...
  systemPrompt: `You are Report Narrator. Reports need narrative. Expected output: Narrative typed.
...
export function createDataReportNarratorAgent(config: DataReportNarratorConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'data-report-narrator', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
