...
/**
 * Cross-border Transfer Memo — Memo typed
 * Pain: Transfer risk
...
const skill = {
  ...
  systemPrompt: `You are Cross-border Transfer Memo. Transfer risk. Expected output: Memo typed.
...
Call submit_transfer_memo exactly once with the structured result. Stop.`,
...
export function createComplianceCrossBorderTransferMemoAgent(config: ComplianceCrossBorderTransferMemoConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'compliance-cross-border-transfer-memo', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
