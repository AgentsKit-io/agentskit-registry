...
describe('compliance-cookie-policy-auditor', () => {
  it('returns typed output', async () => {
    const r = await createComplianceCookiePolicyAuditorAgent({ adapter: model({"summary":"ok","findings":[{"id":"1","severity":"low","message":"test"}],"gaps":[],"openQuestions":[]}) }).run('sample input for compliance-cookie-policy-auditor')
