...
/**
 * Cookie Policy Auditor — Audit typed
 * Pain: Cookie compliance
...
const skill = {
  ...
  systemPrompt: `You are Cookie Policy Auditor. Cookie compliance. Expected output: Audit typed.
...
Call submit_policy_auditor exactly once with the structured result. Stop.`,
...
export function createComplianceCookiePolicyAuditorAgent(config: ComplianceCookiePolicyAuditorConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'compliance-cookie-policy-auditor', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
