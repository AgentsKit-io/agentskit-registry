...
/**
 * Dashboard Spec Author — Spec typed
 * Pain: Dashboard specs ad-hoc
...
const skill = {
  ...
  systemPrompt: `You are Dashboard Spec Author. Dashboard specs ad-hoc. Expected output: Spec typed.
...
export function createDataDashboardSpecAuthorAgent(config: DataDashboardSpecAuthorConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'data-dashboard-spec-author', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
