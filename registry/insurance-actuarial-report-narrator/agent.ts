...
/** Actuarial Report Narrator — v1 validated. Pain: Actuarial opaque */
...
const skill = {
  ...
  systemPrompt: `You are Actuarial Report Narrator. Actuarial opaque. Output: Narrative typed.
...
export function createInsuranceActuarialReportNarratorAgent(config: InsuranceActuarialReportNarratorConfig) {
  ...
  return {
    name: 'insurance-actuarial-report-narrator',
    run,
    asHandle() { return { name: 'insurance-actuarial-report-narrator', run: (t: string) => run(t).then((r) => JSON.stringify(r)) } },
  }
}
