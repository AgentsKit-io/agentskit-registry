...
/**
 * Transcript Cleaner — Clean transcript typed
 * Pain: Messy transcripts
...
const skill = {
  ...
  systemPrompt: `You are Transcript Cleaner. Messy transcripts. Expected output: Clean transcript typed.
...
Call submit_transcript_cleaner exactly once with the structured result. Stop.`,
...
export function createContentTranscriptCleanerAgent(config: ContentTranscriptCleanerConfig) {
  ...
  const submit = (): ToolDefinition =>
    defineZodTool({
      name: 'submit_transcript_cleaner',
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'content-transcript-cleaner', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
