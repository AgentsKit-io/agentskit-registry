...
/** Policy Drafter — v1 validated. Pain: Policies manual */
...
const skill = {
  ...
  systemPrompt: `You are Policy Drafter. Policies manual. Output: Policy draft typed.
...
export function createHrPolicyDrafterAgent(config: HrPolicyDrafterConfig) {
  ...
  return {
    name: 'hr-policy-drafter',
    run,
    asHandle() { return { name: 'hr-policy-drafter', run: (t: string) => run(t).then((r) => JSON.stringify(r)) } },
  }
}
