...
/**
 * Performance Interpreter — Bottlenecks typed
 * Pain: Lighthouse/bundle reports opaque
...
const skill = {
  ...
  systemPrompt: `You are Performance Interpreter. Lighthouse/bundle reports opaque. Expected output: Bottlenecks typed.
...
Call submit_performance_interpreter exactly once with the structured result. Stop.`,
...
export function createCodingPerformanceInterpreterAgent(config: CodingPerformanceInterpreterConfig) {
  ...
  const submit = (): ToolDefinition =>
    defineZodTool({
      name: 'submit_performance_interpreter',
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'coding-performance-interpreter', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
