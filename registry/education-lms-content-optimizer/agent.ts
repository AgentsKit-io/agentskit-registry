...
/**
 * LMS Content Optimizer — Optimizations typed
 * Pain: LMS content weak
...
const skill = {
  ...
  systemPrompt: `You are LMS Content Optimizer. LMS content weak. Expected output: Optimizations typed.
...
Call submit_content_optimizer exactly once with the structured result. Stop.`,
...
export function createEducationLmsContentOptimizerAgent(config: EducationLmsContentOptimizerConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'education-lms-content-optimizer', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
