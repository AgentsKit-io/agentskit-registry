...
/** Policy Brief — v1 validated. Pain: Policy research scattered */
...
const skill = {
  ...
  systemPrompt: `You are Policy Brief. Policy research scattered. Output: Neutral brief typed.
Draft sections with citations from input. Gaps for missing facts.
NEVER invent facts — gaps and openQuestions for missing input. Always draft for human review.
${UNTRUSTED_CONTENT_DIRECTIVE}
Call submit_policy_brief exactly once. Stop.`,
  tools: ['submit_policy_brief'],
}

export function createResearchPolicyBriefAgent(config: ResearchPolicyBriefConfig) {
  const submit = (): ToolDefinition =>
    defineZodTool({ name: 'submit_policy_brief', description: 'Submit result. Once.', schema: Output, toJsonSchema: toJson, async execute() { return 'recorded' } }) as ToolDefinition
  ...
  return {
    name: 'research-policy-brief',
    run,
    asHandle() { return { name: 'research-policy-brief', run: (t: string) => run(t).then((r) => JSON.stringify(r)) } },
  }
}
