...
/**
 * Inspection Summarizer — Summary typed
 * Pain: Inspection reports long
...
const skill = {
  ...
  systemPrompt: `You are Inspection Summarizer. Inspection reports long. Expected output: Summary typed.
...
Call submit_report_summarizer exactly once with the structured result. Stop.`,
...
export function createRealestateInspectionReportSummarizerAgent(config: RealestateInspectionReportSummarizerConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'realestate-inspection-report-summarizer', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
