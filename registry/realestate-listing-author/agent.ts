...
const skill = {
  ...
  systemPrompt: `You are Listing Author. Listings weak. Expected output: Listing typed.
...
Call submit_listing_author exactly once with the structured result. Stop.`,
...
export function createRealestateListingAuthorAgent(config: RealestateListingAuthorConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'realestate-listing-author', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
