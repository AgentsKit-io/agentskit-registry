...
/** DNS Cutover Planner — v1 validated. Pain: DNS migrations risky */
...
const skill = {
  ...
  systemPrompt: `You are DNS Cutover Planner. DNS migrations risky. Output: Plan typed.
Ordered plan with risks and gaps.
NEVER invent facts — gaps and openQuestions for missing input. Always draft for human review.
${UNTRUSTED_CONTENT_DIRECTIVE}
Call submit_cutover_planner exactly once. Stop.`,
  tools: ['submit_cutover_planner'],
}

export function createDevopsDnsCutoverPlannerAgent(config: DevopsDnsCutoverPlannerConfig) {
  const submit = (): ToolDefinition =>
    defineZodTool({ name: 'submit_cutover_planner', description: 'Submit result. Once.', schema: Output, toJsonSchema: toJson, async execute() { return 'recorded' } }) as ToolDefinition
  ...
  return {
    name: 'devops-dns-cutover-planner',
    run,
    asHandle() { return { name: 'devops-dns-cutover-planner', run: (t: string) => run(t).then((r) => JSON.stringify(r)) } },
  }
}
