...
/**
 * Academic Synthesizer — Claims typed + DOI/URL
 * Pain: Paper overload
...
const skill = {
  ...
  systemPrompt: `You are Academic Synthesizer. Paper overload. Expected output: Claims typed + DOI/URL.
...
Call submit_academic_synthesizer exactly once with the structured result. Stop.`,
...
export function createResearchAcademicSynthesizerAgent(config: ResearchAcademicSynthesizerConfig) {
  ...
  const submit = (): ToolDefinition =>
    defineZodTool({
      name: 'submit_academic_synthesizer',
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'research-academic-synthesizer', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
