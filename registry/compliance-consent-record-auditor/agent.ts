...
/**
 * Consent Record Auditor — Audit typed
 * Pain: Consent gaps
...
const skill = {
  ...
  systemPrompt: `You are Consent Record Auditor. Consent gaps. Expected output: Audit typed.
...
Call submit_record_auditor exactly once with the structured result. Stop.`,
...
export function createComplianceConsentRecordAuditorAgent(config: ComplianceConsentRecordAuditorConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'compliance-consent-record-auditor', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
