...
describe('ops-access-request-reviewer', () => {
  it('returns typed output', async () => {
    const r = await createOpsAccessRequestReviewerAgent({ adapter: model({"summary":"ok","findings":[{"id":"1","severity":"low","message":"test"}],"gaps":[],"openQuestions":[]}) }).run('sample input for ops-access-request-reviewer')
