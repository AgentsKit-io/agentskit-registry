...
/**
 * Access Request Reviewer — Review typed
 * Pain: Access grants risky
...
const skill = {
  ...
  systemPrompt: `You are Access Request Reviewer. Access grants risky. Expected output: Review typed.
...
export function createOpsAccessRequestReviewerAgent(config: OpsAccessRequestReviewerConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'ops-access-request-reviewer', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
