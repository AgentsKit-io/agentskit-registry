...
/**
 * Landing CRO Auditor — CRO findings typed
 * Pain: LP not converting
...
const skill = {
  ...
  systemPrompt: `You are Landing CRO Auditor. LP not converting. Expected output: CRO findings typed.
...
Call submit_cro_auditor exactly once with the structured result. Stop.`,
...
export function createMarketingLandingCroAuditorAgent(config: MarketingLandingCroAuditorConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'marketing-landing-cro-auditor', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
