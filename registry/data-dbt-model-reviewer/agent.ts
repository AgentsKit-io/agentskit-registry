...
/**
 * dbt Model Reviewer — Findings typed
 * Pain: dbt quality issues
...
const skill = {
  ...
  systemPrompt: `You are dbt Model Reviewer. dbt quality issues. Expected output: Findings typed.
...
Call submit_model_reviewer exactly once with the structured result. Stop.`,
...
export function createDataDbtModelReviewerAgent(config: DataDbtModelReviewerConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'data-dbt-model-reviewer', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
