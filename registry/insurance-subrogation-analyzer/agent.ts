...
/**
 * Subrogation Analyzer — Analysis typed
 * Pain: Subrogation complex
...
const skill = {
  ...
  systemPrompt: `You are Subrogation Analyzer. Subrogation complex. Expected output: Analysis typed.
...
Call submit_subrogation_analyzer exactly once with the structured result. Stop.`,
...
export function createInsuranceSubrogationAnalyzerAgent(config: InsuranceSubrogationAnalyzerConfig) {
  ...
  const submit = (): ToolDefinition =>
    defineZodTool({
      name: 'submit_subrogation_analyzer',
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'insurance-subrogation-analyzer', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
