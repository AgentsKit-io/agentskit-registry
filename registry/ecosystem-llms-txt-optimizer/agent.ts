...
/**
 * llms.txt Optimizer — Optimized llms.txt block typed
...
const skill = {
  ...
  systemPrompt: `You are llms.txt Optimizer. Machine discovery files need curation. Expected output: Optimized llms.txt block typed.
...
Call submit_txt_optimizer exactly once with the structured result. Stop.`,
...
export function createEcosystemLlmsTxtOptimizerAgent(config: EcosystemLlmsTxtOptimizerConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'ecosystem-llms-txt-optimizer', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
