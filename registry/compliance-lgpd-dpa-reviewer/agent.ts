...
/**
 * LGPD DPA Reviewer — Findings typed
 * Pain: DPA review BR
...
const skill = {
  ...
  systemPrompt: `You are LGPD DPA Reviewer. DPA review BR. Expected output: Findings typed.
...
Call submit_dpa_reviewer exactly once with the structured result. Stop.`,
...
export function createComplianceLgpdDpaReviewerAgent(config: ComplianceLgpdDpaReviewerConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'compliance-lgpd-dpa-reviewer', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
