...
/** Feedback Clusterer — v1 validated. Pain: Feedback scattered */
...
const skill = {
  ...
  systemPrompt: `You are Feedback Clusterer. Feedback scattered. Output: Clusters typed.
Group into themed clusters.
NEVER invent facts — gaps and openQuestions for missing input. Always draft for human review.
${UNTRUSTED_CONTENT_DIRECTIVE}
Call submit_feedback_clusterer exactly once. Stop.`,
...
export function createProductFeedbackClustererAgent(config: ProductFeedbackClustererConfig) {
  const submit = (): ToolDefinition =>
    defineZodTool({ name: 'submit_feedback_clusterer', description: 'Submit result. Once.', schema: Output, toJsonSchema: toJson, async execute() { return 'recorded' } }) as ToolDefinition
  ...
  return {
    name: 'product-feedback-clusterer',
    run,
    asHandle() { return { name: 'product-feedback-clusterer', run: (t: string) => run(t).then((r) => JSON.stringify(r)) } },
  }
}
