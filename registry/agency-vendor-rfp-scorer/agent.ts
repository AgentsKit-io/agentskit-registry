...
/**
 * Vendor RFP Scorer — Scorecard typed
 * Pain: RFP scoring subjective
...
const skill = {
  ...
  systemPrompt: `You are Vendor RFP Scorer. RFP scoring subjective. Expected output: Scorecard typed.
...
Call submit_rfp_scorer exactly once with the structured result. Stop.`,
...
export function createAgencyVendorRfpScorerAgent(config: AgencyVendorRfpScorerConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'agency-vendor-rfp-scorer', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
