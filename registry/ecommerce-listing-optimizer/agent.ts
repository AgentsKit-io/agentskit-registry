...
/**
 * Listing Optimizer — Optimized listing typed
 * Pain: Weak listings
...
const skill = {
  ...
  systemPrompt: `You are Listing Optimizer. Weak listings. Expected output: Optimized listing typed.
...
Call submit_listing_optimizer exactly once with the structured result. Stop.`,
...
export function createEcommerceListingOptimizerAgent(config: EcommerceListingOptimizerConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'ecommerce-listing-optimizer', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
