...
/**
 * Feature Store Documenter — Docs typed
 * Pain: Features undocumented
...
const skill = {
  ...
  systemPrompt: `You are Feature Store Documenter. Features undocumented. Expected output: Docs typed.
...
Call submit_store_documenter exactly once with the structured result. Stop.`,
...
export function createDataFeatureStoreDocumenterAgent(config: DataFeatureStoreDocumenterConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'data-feature-store-documenter', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
