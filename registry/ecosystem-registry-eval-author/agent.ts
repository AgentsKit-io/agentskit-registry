...
/** Registry Eval Author — v1 validated. Pain: Validated agents need eval.ts cases */
...
const skill = {
  ...
  systemPrompt: `You are Registry Eval Author. Validated agents need eval.ts cases. Output: Eval cases typed for @agentskit/eval.
...
export function createEcosystemRegistryEvalAuthorAgent(config: EcosystemRegistryEvalAuthorConfig) {
  const submit = (): ToolDefinition =>
    defineZodTool({ name: 'submit_eval_author', description: 'Submit result. Once.', schema: Output, toJsonSchema: toJson, async execute() { return 'recorded' } }) as ToolDefinition
  ...
  return {
    name: 'ecosystem-registry-eval-author',
    run,
    asHandle() { return { name: 'ecosystem-registry-eval-author', run: (t: string) => run(t).then((r) => JSON.stringify(r)) } },
  }
}
