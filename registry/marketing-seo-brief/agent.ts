...
/**
 * SEO Brief — Keywords/structure typed
 * Pain: Content without SEO intent
...
const skill = {
  ...
  systemPrompt: `You are SEO Brief. Content without SEO intent. Expected output: Keywords/structure typed.
...
Call submit_seo_brief exactly once with the structured result. Stop.`,
...
export function createMarketingSeoBriefAgent(config: MarketingSeoBriefConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'marketing-seo-brief', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
