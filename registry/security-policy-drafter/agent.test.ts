...
describe('security-policy-drafter', () => {
  it('returns typed v1 output', async () => {
    const r = await createSecurityPolicyDrafterAgent({ adapter: model({ title: 'Security Policy Drafter', sections: [{ heading: 'Summary', body: 'content', citations: [] }], gaps: [], openQuestions: [] }) }).run('sample input for security-policy-drafter')
