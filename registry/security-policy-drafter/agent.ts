...
/**
 * Security Policy Drafter — Policy typed
...
const skill = {
  ...
  systemPrompt: `You are Security Policy Drafter. Policies manual. Expected output: Policy typed.
...
export function createSecurityPolicyDrafterAgent(config: SecurityPolicyDrafterConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'security-policy-drafter', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
