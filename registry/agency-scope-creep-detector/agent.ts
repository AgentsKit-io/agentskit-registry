...
/**
 * Scope Creep Detector — Flags vs SOW typed
 * Pain: Scope creep unnoticed
...
const skill = {
  ...
  systemPrompt: `You are Scope Creep Detector. Scope creep unnoticed. Expected output: Flags vs SOW typed.
...
Call submit_creep_detector exactly once with the structured result. Stop.`,
...
export function createAgencyScopeCreepDetectorAgent(config: AgencyScopeCreepDetectorConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'agency-scope-creep-detector', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
