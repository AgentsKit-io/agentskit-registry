...
/**
 * Compliance Checklist — Checklist typed
 * Pain: Compliance ad-hoc
...
const skill = {
  ...
  systemPrompt: `You are Compliance Checklist. Compliance ad-hoc. Expected output: Checklist typed.
...
export function createOpsComplianceChecklistAgent(config: OpsComplianceChecklistConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'ops-compliance-checklist', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
