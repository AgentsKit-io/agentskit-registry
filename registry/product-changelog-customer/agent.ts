...
/**
 * Customer Changelog — Changelog typed
 * Pain: Customer-facing changelog
...
const skill = {
  ...
  systemPrompt: `You are Customer Changelog. Customer-facing changelog. Expected output: Changelog typed.
...
Call submit_changelog_customer exactly once with the structured result. Stop.`,
...
export function createProductChangelogCustomerAgent(config: ProductChangelogCustomerConfig) {
  ...
  const submit = (): ToolDefinition =>
    defineZodTool({
      name: 'submit_changelog_customer',
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'product-changelog-customer', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
