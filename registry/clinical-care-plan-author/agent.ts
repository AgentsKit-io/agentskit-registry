...
/**
 * Care Plan Author — Plan typed
 * Pain: Care plans manual
...
const skill = {
  ...
  systemPrompt: `You are Care Plan Author. Care plans manual. Expected output: Plan typed.
...
export function createClinicalCarePlanAuthorAgent(config: ClinicalCarePlanAuthorConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'clinical-care-plan-author', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
