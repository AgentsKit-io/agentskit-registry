...
/**
 * Industry Benchmark — Metrics typed + source
 * Pain: Benchmark hunting
...
const skill = {
  ...
  systemPrompt: `You are Industry Benchmark. Benchmark hunting. Expected output: Metrics typed + source.
...
Call submit_industry_benchmark exactly once with the structured result. Stop.`,
...
export function createResearchIndustryBenchmarkAgent(config: ResearchIndustryBenchmarkConfig) {
  ...
  const submit = (): ToolDefinition =>
    defineZodTool({
      name: 'submit_industry_benchmark',
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'research-industry-benchmark', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
