...
/**
 * IAM Policy Reviewer — Findings typed
 * Pain: Over-permissive IAM
...
const skill = {
  ...
  systemPrompt: `You are IAM Policy Reviewer. Over-permissive IAM. Expected output: Findings typed.
...
Call submit_policy_reviewer exactly once with the structured result. Stop.`,
...
export function createDevopsIamPolicyReviewerAgent(config: DevopsIamPolicyReviewerConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'devops-iam-policy-reviewer', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
