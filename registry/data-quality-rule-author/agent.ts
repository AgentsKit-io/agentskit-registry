...
/**
 * Data Quality Rule Author — Rules typed
 * Pain: DQ rules manual
...
const skill = {
  ...
  systemPrompt: `You are Data Quality Rule Author. DQ rules manual. Expected output: Rules typed.
...
Call submit_rule_author exactly once with the structured result. Stop.`,
...
export function createDataQualityRuleAuthorAgent(config: DataQualityRuleAuthorConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'data-quality-rule-author', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
