...
/**
 * Threat Intel Brief — Brief typed
 * Pain: Threat intel scattered
...
const skill = {
  ...
  systemPrompt: `You are Threat Intel Brief. Threat intel scattered. Expected output: Brief typed.
...
Call submit_intel_brief exactly once with the structured result. Stop.`,
...
export function createSecurityThreatIntelBriefAgent(config: SecurityThreatIntelBriefConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'security-threat-intel-brief', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
