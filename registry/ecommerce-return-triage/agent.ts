...
const skill = {
  ...
  systemPrompt: `You are Return Triage. Returns slow. Expected output: Triage typed.
...
Call submit_return_triage exactly once with the structured result. Stop.`,
...
export function createEcommerceReturnTriageAgent(config: EcommerceReturnTriageConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'ecommerce-return-triage', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
