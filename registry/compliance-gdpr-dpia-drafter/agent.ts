...
const skill = {
  ...
  systemPrompt: `You are GDPR DPIA Drafter. DPIA manual. Expected output: DPIA typed.
...
Call submit_dpia_drafter exactly once with the structured result. Stop.`,
...
export function createComplianceGdprDpiaDrafterAgent(config: ComplianceGdprDpiaDrafterConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'compliance-gdpr-dpia-drafter', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
