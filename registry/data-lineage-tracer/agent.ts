...
/**
 * Lineage Tracer — Lineage typed
 * Pain: Lineage unknown
...
const skill = {
  ...
  systemPrompt: `You are Lineage Tracer. Lineage unknown. Expected output: Lineage typed.
...
Call submit_lineage_tracer exactly once with the structured result. Stop.`,
...
export function createDataLineageTracerAgent(config: DataLineageTracerConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'data-lineage-tracer', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
