...
/**
 * Fulfillment SLA Monitor — Alerts typed
 * Pain: SLA breaches
...
const skill = {
  ...
  systemPrompt: `You are Fulfillment SLA Monitor. SLA breaches. Expected output: Alerts typed.
...
Call submit_sla_monitor exactly once with the structured result. Stop.`,
...
export function createEcommerceFulfillmentSlaMonitorAgent(config: EcommerceFulfillmentSlaMonitorConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'ecommerce-fulfillment-sla-monitor', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
