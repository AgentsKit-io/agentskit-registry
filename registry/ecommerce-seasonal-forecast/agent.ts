...
/**
 * Seasonal Forecast — Forecast typed
 * Pain: Seasonal demand
...
const skill = {
  ...
  systemPrompt: `You are Seasonal Forecast. Seasonal demand. Expected output: Forecast typed.
...
Call submit_seasonal_forecast exactly once with the structured result. Stop.`,
...
export function createEcommerceSeasonalForecastAgent(config: EcommerceSeasonalForecastConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'ecommerce-seasonal-forecast', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
