...
/**
 * Churn Risk Scorer — Risk score typed
 * Pain: Silent churn
...
const skill = {
  ...
  systemPrompt: `You are Churn Risk Scorer. Silent churn. Expected output: Risk score typed.
...
export function createSupportChurnRiskScorerAgent(config: SupportChurnRiskScorerConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'support-churn-risk-scorer', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
