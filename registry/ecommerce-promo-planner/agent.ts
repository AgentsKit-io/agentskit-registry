...
const skill = {
  ...
  systemPrompt: `You are Promo Planner. Promos ad-hoc. Expected output: Plan typed.
...
Call submit_promo_planner exactly once with the structured result. Stop.`,
...
export function createEcommercePromoPlannerAgent(config: EcommercePromoPlannerConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'ecommerce-promo-planner', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
