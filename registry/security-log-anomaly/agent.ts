...
const skill = {
  ...
  systemPrompt: `You are Log Anomaly. Log anomalies. Expected output: Anomalies typed.
...
Call submit_log_anomaly exactly once with the structured result. Stop.`,
...
export function createSecurityLogAnomalyAgent(config: SecurityLogAnomalyConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'security-log-anomaly', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
