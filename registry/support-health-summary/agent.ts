...
/**
 * Account Health Summary — Health summary typed
 * Pain: Account reviews manual
...
const skill = {
  ...
  systemPrompt: `You are Account Health Summary. Account reviews manual. Expected output: Health summary typed.
...
Call submit_health_summary exactly once with the structured result. Stop.`,
...
export function createSupportHealthSummaryAgent(config: SupportHealthSummaryConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'support-health-summary', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
