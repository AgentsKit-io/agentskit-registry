...
/**
 * Review Responder — Reply draft typed
 * Pain: Review replies slow
...
const skill = {
  ...
  systemPrompt: `You are Review Responder. Review replies slow. Expected output: Reply draft typed.
...
Call submit_review_responder exactly once with the structured result. Stop.`,
...
export function createEcommerceReviewResponderAgent(config: EcommerceReviewResponderConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'ecommerce-review-responder', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
