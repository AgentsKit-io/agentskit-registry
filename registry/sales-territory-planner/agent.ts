...
/**
 * Territory Planner — Plan typed
 * Pain: Territory planning
...
const skill = {
  ...
  systemPrompt: `You are Territory Planner. Territory planning. Expected output: Plan typed.
...
Call submit_territory_planner exactly once with the structured result. Stop.`,
...
export function createSalesTerritoryPlannerAgent(config: SalesTerritoryPlannerConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'sales-territory-planner', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
