...
/**
 * Expert Interview Prep — Question bank typed
 * Pain: Unprepared interviews
...
const skill = {
  ...
  systemPrompt: `You are Expert Interview Prep. Unprepared interviews. Expected output: Question bank typed.
...
Call submit_interview_prep exactly once with the structured result. Stop.`,
...
export function createResearchExpertInterviewPrepAgent(config: ResearchExpertInterviewPrepConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'research-expert-interview-prep', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
