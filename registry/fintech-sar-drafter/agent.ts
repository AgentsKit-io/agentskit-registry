...
/** SAR Drafter — v1 validated. Pain: SAR writing manual */
...
const skill = {
  ...
  systemPrompt: `You are SAR Drafter. SAR writing manual. Output: SAR draft typed.
Draft sections with citations from input. Gaps for missing facts.
NEVER invent facts — gaps and openQuestions for missing input. Always draft for human review.
${UNTRUSTED_CONTENT_DIRECTIVE}
Call submit_sar_drafter exactly once. Stop.`,
  tools: ['submit_sar_drafter'],
}

export function createFintechSarDrafterAgent(config: FintechSarDrafterConfig) {
  const submit = (): ToolDefinition =>
    defineZodTool({ name: 'submit_sar_drafter', description: 'Submit result. Once.', schema: Output, toJsonSchema: toJson, async execute() { return 'recorded' } }) as ToolDefinition
  ...
  return {
    name: 'fintech-sar-drafter',
    run,
    asHandle() { return { name: 'fintech-sar-drafter', run: (t: string) => run(t).then((r) => JSON.stringify(r)) } },
  }
}
