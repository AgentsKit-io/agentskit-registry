...
/**
 * K8s Manifest Reviewer — Findings typed
 * Pain: YAML risks
...
const skill = {
  ...
  systemPrompt: `You are K8s Manifest Reviewer. YAML risks. Expected output: Findings typed.
...
Call submit_manifest_reviewer exactly once with the structured result. Stop.`,
...
export function createDevopsK8sManifestReviewerAgent(config: DevopsK8sManifestReviewerConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'devops-k8s-manifest-reviewer', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
