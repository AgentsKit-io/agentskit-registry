...
/**
 * Due Diligence Pack — DD pack typed claim→URL
 * Pain: M&A/vendor DD manual
...
const skill = {
  ...
  systemPrompt: `You are Due Diligence Pack. M&A/vendor DD manual. Expected output: DD pack typed claim→URL.
...
Call submit_due_diligence exactly once with the structured result. Stop.`,
...
export function createResearchDueDiligenceAgent(config: ResearchDueDiligenceConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'research-due-diligence', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
