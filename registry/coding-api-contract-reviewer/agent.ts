...
/**
 * API Contract Reviewer — Breaking/non-breaking diff typed
 * Pain: Breaking API changes
...
const skill = {
  ...
  systemPrompt: `You are API Contract Reviewer. Breaking API changes. Expected output: Breaking/non-breaking diff typed.
...
Call submit_contract_reviewer exactly once with the structured result. Stop.`,
...
export function createCodingApiContractReviewerAgent(config: CodingApiContractReviewerConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'coding-api-contract-reviewer', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
