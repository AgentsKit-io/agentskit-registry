...
/**
 * Accreditation Evidence — Evidence typed
 * Pain: Accreditation prep
...
const skill = {
  ...
  systemPrompt: `You are Accreditation Evidence. Accreditation prep. Expected output: Evidence typed.
...
Call submit_accreditation_evidence exactly once with the structured result. Stop.`,
...
export function createEducationAccreditationEvidenceAgent(config: EducationAccreditationEvidenceConfig) {
  ...
  const submit = (): ToolDefinition =>
    defineZodTool({
      name: 'submit_accreditation_evidence',
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'education-accreditation-evidence', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
