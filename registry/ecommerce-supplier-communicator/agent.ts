...
/** Supplier Communicator — v1 validated. Pain: Supplier emails */
...
const skill = {
  ...
  systemPrompt: `You are Supplier Communicator. Supplier emails. Output: Email draft typed.
Draft sections with citations from input. Gaps for missing facts.
NEVER invent facts — gaps and openQuestions for missing input. Always draft for human review.
${UNTRUSTED_CONTENT_DIRECTIVE}
Call submit_supplier_communicator exactly once. Stop.`,
...
export function createEcommerceSupplierCommunicatorAgent(config: EcommerceSupplierCommunicatorConfig) {
  const submit = (): ToolDefinition =>
    defineZodTool({ name: 'submit_supplier_communicator', description: 'Submit result. Once.', schema: Output, toJsonSchema: toJson, async execute() { return 'recorded' } }) as ToolDefinition
  ...
  return {
    name: 'ecommerce-supplier-communicator',
    run,
    asHandle() { return { name: 'ecommerce-supplier-communicator', run: (t: string) => run(t).then((r) => JSON.stringify(r)) } },
  }
}
