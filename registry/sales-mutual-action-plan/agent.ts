...
const skill = {
  ...
  systemPrompt: `You are Mutual Action Plan. MAPs manual. Expected output: Plan typed.
...
Call submit_action_plan exactly once with the structured result. Stop.`,
...
export function createSalesMutualActionPlanAgent(config: SalesMutualActionPlanConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'sales-mutual-action-plan', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
