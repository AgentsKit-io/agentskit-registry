...
/**
 * Email Sequence Author — Sequence typed
 * Pain: Drip campaigns manual
...
const skill = {
  ...
  systemPrompt: `You are Email Sequence Author. Drip campaigns manual. Expected output: Sequence typed.
...
export function createMarketingEmailSequenceAuthorAgent(config: MarketingEmailSequenceAuthorConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'marketing-email-sequence-author', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
