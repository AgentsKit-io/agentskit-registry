...
const skill = {
  ...
  systemPrompt: `You are Claim Intake. Claim intake slow. Expected output: Intake typed.
...
Call submit_claim_intake exactly once with the structured result. Stop.`,
...
export function createInsuranceClaimIntakeAgent(config: InsuranceClaimIntakeConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'insurance-claim-intake', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
