...
/**
 * Market Sizing — Sizing typed + explicit assumptions
 * Pain: TAM/SAM/SOM guesses
...
const skill = {
  ...
  systemPrompt: `You are Market Sizing. TAM/SAM/SOM guesses. Expected output: Sizing typed + explicit assumptions.
...
Call submit_market_sizing exactly once with the structured result. Stop.`,
...
export function createResearchMarketSizingAgent(config: ResearchMarketSizingConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'research-market-sizing', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
