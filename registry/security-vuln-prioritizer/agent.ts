...
/**
 * Vuln Prioritizer — Priority typed
 * Pain: Vuln backlog
...
const skill = {
  ...
  systemPrompt: `You are Vuln Prioritizer. Vuln backlog. Expected output: Priority typed.
...
Call submit_vuln_prioritizer exactly once with the structured result. Stop.`,
...
export function createSecurityVulnPrioritizerAgent(config: SecurityVulnPrioritizerConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'security-vuln-prioritizer', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
