...
/**
 * Email Triage — Classification typed
 * Pain: Inbox overload
...
const skill = {
  ...
  systemPrompt: `You are Email Triage. Inbox overload. Expected output: Classification typed.
...
Call submit_email_triage exactly once with the structured result. Stop.`,
...
export function createProductivityEmailTriageAgent(config: ProductivityEmailTriageConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'productivity-email-triage', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
