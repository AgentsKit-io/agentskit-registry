...
/**
 * Learning Path Builder — Path typed
 * Pain: Learning paths manual
...
const skill = {
  ...
  systemPrompt: `You are Learning Path Builder. Learning paths manual. Expected output: Path typed.
...
Call submit_path_builder exactly once with the structured result. Stop.`,
...
export function createHrLearningPathBuilderAgent(config: HrLearningPathBuilderConfig) {
  ...
  return {
    ...
    run,
    asHandle() {
      return { name: 'hr-learning-path-builder', run: async (task: string) => JSON.stringify(await run(task)) }
    },
  }
}
